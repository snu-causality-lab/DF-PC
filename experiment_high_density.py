...
class PCStableHighDensityWrapper(PCStable):
    """
    Custom wrapper to support standard and pure DF-PC execution in high density regimes.
    """
...
def run_single_rep(n, m, d, g_type, d_type, rep, algo_mode):
    ...
    runner = PCStableHighDensityWrapper(alpha=0.01, ci_tester=tester, algo_mode=algo_mode)
...
def main():
    ...
    NODES = [30]
    SAMPLES = [5000]
    AVG_DEGREES = [6]  # Specific intense density test
    GRAPH_TYPES = ['er', 'sf']
    ...
    csv_path = output_dir / "experiment_high_density_summary.csv"
    raw_path = output_dir / "experiment_high_density_raw.csv"
    ...
    df_raw.to_csv(output_dir / "experiment_2_raw.csv", index=False)
    ...
    df_summary.to_csv(output_dir / "experiment_2_summary.csv", index=False)
