...
class ErrorOracleCITester:
    def __init__(self, true_graph, error_rate=0.0):
    ...
    def ci_test(self, data, i, j, cond_set):
        ...
        # Inject error on low-order tests (|Z| <= 1)
        if len(cond_set) <= 1 and random.random() < self.error_rate:
...
def run_single_rep(n, d, p_err, rep, algo_mode):
    ...
    tester = ErrorOracleCITester(G, error_rate=p_err)
    runner = PCStableRebuttal(alpha=0.01, ci_tester=tester, algo_mode=algo_mode)
    
    runner.run(dummy_data)
    adj_est = runner.adj_mat if hasattr(runner, 'adj_mat') else runner.run(dummy_data)[0]
    
    # We must properly pull the adjacency structure - above runner.run yields it. Correcting locally:
...
def main():
    ...
    csv_path = output_dir / "experiment_5_summary.csv"
    raw_path = output_dir / "experiment_5_raw.csv"
