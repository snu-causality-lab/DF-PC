...
class PCStableRuntimeWrapper(PCStable):
    """
    Custom wrapper to support runtime scaling benchmarks.
    """
...
def run_single_rep(setup_name, n, m, d, g_type, d_type, rep, algo_mode):
    ...
    runner = PCStableRuntimeWrapper(alpha=0.01, ci_tester=tester, algo_mode=algo_mode)
...
def main():
    ...
    csv_path = output_dir / "experiment_runtime_scaling_summary.csv"
    raw_path = output_dir / "experiment_runtime_scaling_raw.csv"
    ...
    df_raw.to_csv(output_dir / "experiment_3_raw.csv", index=False)
    ...
    df_summary.to_csv(output_dir / "experiment_3_summary.csv", index=False)
