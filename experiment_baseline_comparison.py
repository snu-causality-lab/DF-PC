...
class PCStableBaselineWrapper(PCStable):
    """
...
def run_single_rep(n, m, d, g_type, d_type, rep, algo_mode):
    ...
    runner = PCStableBaselineWrapper(alpha=0.01, ci_tester=tester, algo_mode=algo_mode)
...
def main():
    ...
    csv_path = output_dir / "experiment_baseline_comparison_summary.csv"
    raw_path = output_dir / "experiment_baseline_comparison_raw.csv"
    ...
    df_raw.to_csv(output_dir / "experiment_1_raw.csv", index=False)
    ...
    df_summary.to_csv(output_dir / "experiment_1_summary.csv", index=False)
