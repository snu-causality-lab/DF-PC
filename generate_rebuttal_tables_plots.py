...
def save_plots(fig, filename_base):
    ...
    for ext in ["png", "pdf"]:
        # Mapping compatibility between paper experiment numbers (7,8,9,10,11) and legacy figures (1,2,3,4,5)
        name_map = {
            "exp7_": "exp1_",
            "exp8_": "exp2_",
            "exp9_": "exp3_",
        ...
        fig.savefig(f"{folder}/plots/{filename_base}.{ext}", dpi=300)
        ...
        if alt_name != filename_base:
            fig.savefig(f"{folder}/plots/{alt_name}.{ext}", dpi=300)
    ...
    def plot_2x2(df_raw, metric, ylabel, filename_base):
        ...
        for r, g in enumerate(graphs):
            for c, d in enumerate(dtypes):
                ...
                # Ensure correct ordering and coloring
...
if os.path.exists(f"{folder}/experiment_4_summary.csv") and os.path.exists(f"{folder}/experiment_4_raw.csv"):
    ...
    # Setup A: Node Scaling — one subplot per Node count
    ...
    # Setup B: Density Scaling — one subplot per Degree
...
tex_file.close()
print("Plots generated successfully in PDF and PNG with UAI style formatting.")
